...
Each ruling says what was decided, why, and what it costs if wrong. Later rulings supersede earlier ones where they say so.

- Ruling: split `XFAIL` into `XFAIL_BUCKET` (T1: bucket/flags test) and `XFAIL_PAYOUT` (created in T10 for the payout test). VRTV/TWO/BLD are not in `XFAIL_BUCKET` unless they actually fail it. — A shared dict makes a correct case XPASS under strict xfail. — If wrong: a few cases are mislabeled xfail, visible in test output.
...
- Ruling (T1 XFAIL remap, applied in the Task 1 fix round): HLTH→4, XTO→6, LEAP→8, IMCL→3 (with GOLDEN_MANUAL), HYH→7, KCI→9 (window from Form 25), others as reported.
- Ruling: Task 13 runs the acceptance twice, with the pinned 2026-05-26 map and with SEC's current map, and the criteria must hold for both. — Production will run on a fresh map; Task 3's rulings (member-name tier first, frequency score > 0) exist for exactly that. — If wrong: an acceptance pass that only holds for a stale map.
- Ruling (supersedes "member tier first" / "ticker-ignore" / "skip score-0"): EFTS keeps its place ahead of the member-name search, `names_agree` needs min(2,|A|,|B|) shared 3+-letter words, names are compared over ±30 days (`names_near`), and the frequency tier keeps score-0 winners, marked `efts_frequency_name_mismatch`. — The Task 1 implementer ran the first design against the fixtures: FST resolved to Forest City, HYH failed on a 1-day rename boundary, and HMA/HLTH went unresolved. I checked the new design: name agreement is right on all 31 golden cases (mismatch exactly on IMCL FST BWC HMA LEAP HLTH), and the names/evidence snippet tests pass. — If wrong: two-word agreement is too strict for a real match with one shared distinctive word plus differing suffix words; that case falls through to the later tiers and is visible as unknown or a mismatch flag.
...
- Ruling (T7 Important, plan-mandated): `_rename_or_transfer` returns None when an 8-K dated within ±30 days of the observed date shows the registrant was acquired: item 5.01, or 2.01 together with 3.01 or 3.03 (the same M&A signals Task 6 uses). The merger fingerprint then decides. — A takeover must not read as a continuation. A bare 2.01 (a disposition) does not block, so Halyard's April 2018 division sale is unaffected and is 59 days out anyway. — If wrong: a rename that coincides with a change in control (a reverse merger) goes to the merger path, where the payout abstains to par, the same 0 return.
- Ruling (T8 Important, plan-mandated): `is_spac(sub, on)` = the name at the date matches `_SPAC_NAME`, OR (SIC == 6770 AND no formerNames entry ended on or before `on`). The docstring must say exactly that. — A de-SPAC always renames (Peak Bio was "Ignyte Acquisition Corp."), while all four real SPAC liquidations (FST BWC HMA LEAP) have SIC 6770 and empty formerNames (checked in the fixtures). — If wrong: a SPAC that renamed without "Acquisition Corp" in its new name and then liquidated falls to the ordinary rules (compliance/liquidation marks).
- Ruling (T9 Q1): every code 570 goes through `_default_without_fingerprint`, and the helper's order becomes: (1) 2.01 + dereg → merger 233; (2) anchor 3.01 notice text cites a listing deficiency → compliance (580 if NT 10-K/Q in the prior year, else 570); (3) merger-evidence filing (window from the Form 25 date) → merger 231; (4) NT 10-K/Q in the prior year → 580; (5) unknown (+ no_evidence_default, deregistered). The 580 logic lives only in the helper. — An explicit deficiency notice is stronger evidence than a proxy up to 400 days old (a merger that fell through can precede a real compliance delisting); a completed deal's 3.01 notice has no deficiency wording (KCI). — If wrong: a take-under whose delisting notice also cites a deficiency is marked distress.
...
- Ruling (T11 Minors 3, 5, 7, 8, 9): lazy-import the script helper inside the builder's capture function and state PYTHONPATH=src:. in its docstring; add the listed reconcile unit tests (election cash leg, neither fits, stock/cash_and_stock ignored, fitting regex beats LLM, regex ≤ 0); `payout_gate.DEFAULT_TOL = 0.15` used by the CLI default and the golden test; a full builder run without --raw-tiingo-dir skips the LLM capture with a notice and carries existing keys forward (--llm-only still requires it); election stock rows keep payout_src "llm_election_stock" and the LLM confidence if build_dlret_table then fills payout_source without inventing a payout (verify; else skip and report). Minor 6 (whole XFAIL_PAYOUT removed) was the controller's ruling: no change.
- Ruling (T11 FR1 concern 1): accept — LLM "cash" terms that carry a stock_ratio go only to the cash+stock gate, as before Task 11 (contradictory labels need the full terms to reconcile). — If wrong: such a row whose cash alone matches the close lands at par.
- Ruling (for Task 12): gate_payouts records a per-key flag when the cash+stock gate drops terms for any reason other than csv_override (e.g. `terms_gate_failed:fail_sanity`), so review.csv lists merger rows the rules left at par. — Today those drops show only as counters. — Cost if wrong: a few extra review rows.
- Ruling (for Task 12, election flag): when reconcile settles an election by either leg and the regex value equals the LLM cash leg within 1%, the `payout_gate_failed:*` flag is not recorded (the regex read a real leg, as in concern 1); a regex value that matches neither leg keeps the flag (TWO-style misread stays listed). Update test_payout_gate.py:105 accordingly. — The re-reviewer found BLD-like rows would be listed for review though settled. — If wrong: an election where the regex read the cash leg but the cash leg is itself wrong goes unlisted (the stock leg still had to reconcile).
- Ruling (Task 12, review.csv columns): cik comes from the DelistRecord and anchor_8k from its evidence (the same value as delist_classifications.csv's anchor_8k_items); join table rows to all_records by (ticker.upper(), observed_delist_date). Flags come from `gated.flags` (payout_flags no longer exists). — The DLRET table has neither column. — Cost if wrong: none.
...
- Three definitions of "merger fingerprint" (M_A_ITEMS, rename guard, `control`); `_pick_8k_near(...) or backscan` lets any nearest 8-K block the backscan. Left; revisit if Task 13 shows a case.
