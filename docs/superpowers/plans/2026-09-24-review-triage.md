...
**`no_dlret` (Ruling, Task 2 review):** before decisions are applied, `triage` appends the token `no_dlret` to every delisting row (non-blank `bucket`; Form 25 review items such as `form25_unmatched` carry a `delist_date` but no bucket and are not delisting rows) whose DLRET is blank (None, NaN or `""`). `no_dlret` is in the catalog as `fix`, acceptable. So a delisting with no delisting return stays in `review.csv` as `fix` until a person supplies the value (an override CSV) or accepts `no_dlret` itself; accepting its other flags never clears it. `no_dlret` appears only in `review.csv`/`review_summary.csv`, never in `delistings.csv`.

**`row_severity(row)`:** the most severe of the row's tokens' catalog severities (`fix` > `check` > `info`). A row with no tokens is `info`.
